...
export function PitchSourceV2() {
  ...
  return <>
    ...
    {open && <div className="dv2-source-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><aside className="dv2-source-panel" role="dialog" aria-modal="true" aria-labelledby="dv2-source-title"><div className="dv2-source-panel-head"><span className="dv2-label">Source record / 001</span><button ref={closeRef} className="dv2-icon-button" type="button" onClick={() => setOpen(false)} aria-label="Close original source"><X size={20} /></button></div><h2 id="dv2-source-title">The original<br />signal.</h2>{VERIFIED_X_POST_URL ? <a className="dv2-source-direct" href={VERIFIED_X_POST_URL} target="_blank" rel="noopener noreferrer">Open original on X <ExternalLink size={15} /></a> : <p className="dv2-source-note">Original post link unavailable.</p>}{embedUrl ? <div className="dv2-source-embed"><iframe title="Original Dissent source post" src={embedUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div> : null}<p className="dv2-source-note">Dissent preserves a verified source link without inventing post text, date or engagement data.</p>{VERIFIED_X_POST_URL && <button className="dv2-source-copy" type="button" onClick={() => void navigator.clipboard?.writeText(VERIFIED_X_POST_URL)}><Copy size={14} /> Copy source URL</button>}</aside></div>}
  </>;
}
